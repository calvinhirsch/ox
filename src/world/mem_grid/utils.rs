...
pub fn cubed<T: Copy + Mul<Output = T>>(x: T) -> T { x * x * x }
...
pub fn index_for_pos_in_tlc(pos_in_tlc: Point3<u32>, chunk_size: usize, n_chunk_lvls: usize, chunk_lvl: usize, lod: usize) -> usize {
    let mut idx = 0;
    ...
    idx += index_for_pos(
        (pos_in_tlc % last_chunk_lvl_size as u32).to_vec().cast::<usize>().unwrap(),
        last_chunk_lvl_size
    );
